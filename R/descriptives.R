...
Descriptives <- function(jaspResults, dataset, options) {
  ...
  if (options[["scatterPlot"]]) {
    if(is.null(jaspResults[["scatterPlots"]])) {
      ...
      jaspResults[["scatterPlots"]]$dependOn(c("splitby", "scatterPlot", "graphTypeAbove", "graphTypeRight", "regressionLine",
                                               "regressionLineCi", "regressionLineCiValue", "regressionType", "showLegend",
                                               "colorPalette"))
  ...
  # Heatmap
  if (options[["heatmapHorizontalAxis"]] != "" && options[["heatmapVertical"]] != "") {
    if(is.null(jaspResults[["heatmaps"]])) {
      jaspResults[["heatmaps"]] <- createJaspContainer(gettext("Heatmaps"))
      jaspResults[["heatmaps"]]$dependOn(c("heatmapHorizontalAxis", "heatmapVertical",
    ...
    if (is.null(jaspContainer[[plotName]])) {
      ...
      if (is.null(errorMessage)) {
        ...
        p <- try(jaspGraphs::JASPScatterPlot(
          ...
          addSmoothCI       = options[["regressionLineCi"]],
          smoothCIValue     = options[["regressionLineCiValue"]],
...
.descriptivesHeatmaps <- function(container, dataset, variables, options) {
  axesNames <- c(options[["heatmapHorizontalAxis"]], options[["heatmapVertical"]])
